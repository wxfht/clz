...
class Model {
    ...
    public function query($sql,$parse=false) {
        if(!is_bool($parse) && !is_array($parse)) {
        	...
        	file_put_contents(ROOT_PATH.'Runtime/Logs/wx/category.log',date('Y/m/d H:i:s')."---参数---01--返回：--".$sql.PHP_EOL, FILE_APPEND);
        $sql  =   $this->parseSql($sql,$parse);
        	file_put_contents(ROOT_PATH.'Runtime/Logs/wx/category.log',date('Y/m/d H:i:s')."---参数---02--返回：--".$sql.PHP_EOL, FILE_APPEND);
