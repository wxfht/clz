...
//		M('goods')->field('image')->where( array('goods_id' => ) )->find();
	...
	**/
    ...
    // 小程序解密
   	public function decryptData($encryptedData, $iv, $token,$sessionKey)
    {
		...
		$session_key=($session_key)?$session_key:$sessionKey;
	...
	public function applogin_do()
	{
		...
		$iv = $gpc['iv'];
		$encryptedData = $gpc['encrypteddata'];
		$res = $this->decryptData($encryptedData, $iv, $token,$session_key);
		$unionid = $caceh_data['wepro_unionid_'.$token]?$caceh_data['wepro_unionid_'.$token]:trim($res->unionId);
