...

			{
...

				{
...

					{
						...
						if( $vv != 25 && date('H')<=$vv )//htong 限制整点秒杀 && date('H')<=$vv
...
//				}
                ...
                {
                    if(!empty($val['pick_start_time']) || !empty($val['pick_end_time'])){
                        if(date('H:s')>$val['pick_start_time'] && date('H:s')<$val['pick_end_time'])
                        {
                            $val['pick_up_modify'] = date('Y-m-d', $today_time);
                        }else{
                            $val['pick_up_modify'] = date('Y-m-d', $today_time+86400);
...

	{
		...
		//整点秒杀begin
		if($gpc['is_seckill'] == 1){
			$gpc['is_seckill'] = ($gpc['is_seckill']>= date('H'))?$gpc['is_seckill']:0;//htong 限制整点秒杀
		}
...

		{
			...
			//htong 更改整点 抢购限制整点 开始
			// $ed_time = $bg_time + 3600;
			
			$time=time();

			if($gid == 0 && $keyword == ''){
				$where .= "  and gc.begin_time <={$bg_time} and   gc.end_time >{$bg_time}  ";
			} else {

				$where .= "  and gc.begin_time <={$bg_time} and   gc.end_time >{$bg_time}  ";

			}
			if($seckill_time == date('H')){
				$where .= "and gc.begin_time <={$time} and gc.end_time >={$time}";
			}
			
			//htong 更改整点 抢购限制整点 结束
...

		{
			...
			$where .= " and gc.is_new_buy=0  ";//htong and gc.is_spike_buy = 0 删除
