...
class PingoodsModel {
	...
	public function get_new_community_index_goods($head_id=0,$gid='', $fields='*', $where='1=1',$offset=0,$perpage=10,$order='g.istop DESC, g.settoptime DESC,g.index_sort desc,g.id desc ')
	{
		...
		$list_pingoods = M()->query($sql_pingoods);
		file_put_contents(ROOT_PATH.'Runtime/Logs/wx/category.log',date('Y/m/d H:i:s')."---参数---".$sql_pingoods."--返回：--".json_encode($list_pingoods).PHP_EOL, FILE_APPEND);
		return $list_pingoods;
	}
	...
	public function get_weindex_share_image($community_id,$wepro_qrcode,$avatar)
	{
		...
		switch ($imagesize['mime']) {
			...
			case 'image/jpeg':
				...
				break;
			case 'image/gif':
				$dst = imagecreatefromgif ($bg_img);
				break;
			case 'image/wbmp':
				$dst = imagecreatefromwbmp ($bg_img);
				break;
		}
