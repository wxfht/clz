...
class ReportsController extends CommonController{
    ...
    public function index()
	{
		...
		if (empty($_GPC['reports_index']) || $_GPC['reports_index']=='0'){
			...
			foreach($day_info2 as $key =>$day) {
				...
				$sql = "select r.* from oscshop_lionfish_comshop_order_refund as r left join oscshop_lionfish_comshop_order as o on o.order_id = r.order_id  where o.date_added > ".$day['egt']." and o.date_added < ".$day['lt']." ".$condition." and r.state = 3 GROUP BY r.order_id";
					$day_info3 = M()->query($sql);
					...
					$daytuikuan =0;
					//退款单数
				    $daytui = count($day_info3);
					//退款金额
					foreach($day_info3 as $v){
						$daytuikuan += $v['ref_money'];
					}	
		...
		//其他月时间
		if($_GPC['reports_index'] == 4){
			$lastmonth['egt'] = strtotime($_GPC['data_month'].'-01 00:00:00');
			$lastmonth['lt'] = strtotime("+1 months",$lastmonth['egt'])-1;
			...
			$tabid = 4;
			$this->data_month = $_GPC['data_month'];
		}
	...
	private function head_order_analys($keyword,$searchtime , $starttime , $endtime)//  htong 修改搜索
	{
		$_GPC = I('request.');
	...
	*/
	public function active_users(){
		...
		$psize = 20;
		//开始时间
		$starttime = isset($_GPC['time']['start']) ? strtotime($_GPC['time']['start']) : strtotime(date('1970-01-01 00:00:00'));
		//结束时间
		$endtime=isset($_GPC['time']['end'])?strtotime($_GPC['time']['end']):strtotime(date('Y-m-d 23:59:59'));
		//拼接条件
		$where['date_added'] = array(array('egt',$starttime), array('elt', $endtime));
		$where['order_status_id'] = array('in','1,2,4,6,11,14,12,13');
		//查询获取总条数
		$count_arr = M('lionfish_comshop_order')->alias('o')
		->join('oscshop_lionfish_comshop_member as m on o.member_id = m.member_id')	
		->field('m.member_id,sum(o.total) as total,m.username')
		->where($where)
		->group('m.username')
		->order('total DESC')
		->select();
		...
		//获取信息
		$data =M('lionfish_comshop_order')->alias('o')
			->join('oscshop_lionfish_comshop_member as m on o.member_id = m.member_id')	
			->field('m.member_id,sum(o.total) as total,m.username')
			->where($where)
			->limit($pindex-1,20)
		...
		{	//导出栏目
			$columns = array(
						array('title' => 'ID', 'field' => 'member_id', 'width' => 32),
						array('title' => '姓名', 'field' => 'username', 'width' => 32),
						array('title' => '总金额', 'field' => 'total', 'width' => 32),
			);
			//表名
			$title = '会员活跃表';
			//调用导出方法
		...
		//调用页码方法
		$pager = pagination($count, $pindex, $psize,$_SERVER["QUERY_STRING"].'&page=*');
