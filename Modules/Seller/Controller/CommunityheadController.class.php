...
class CommunityheadController extends CommonController {
	...
	public function goodslist()
	{
		...
		// $this->all_goods_ids = $all_goods_ids;//htong禁用
		...
		// $in_goods_ids  = array_merge($ch_goods_arr, $all_goods_ids);//htong禁用
		$in_goods_ids = $ch_goods_arr;
		$this->all_goods_ids = $ch_goods_arr;//htong
	...
	*/
	public function modify_amount(){
		$tixian = M('lionfish_community_head_tixian_order')->where(['id'=>$_POST['id']])->find();

		$money =$tixian['money'];

		if($_POST['id'] && $_POST['money']){
			$data=array(
				'id' => $_POST['id'],
				'money' => $_POST['money'],
				'remarks' => $_POST['remarks'],
			);
		$re = M('lionfish_community_head_tixian_order')->save($data);
		}
		
		if($re){
			file_put_contents('Runtime/Logs/Seller/tixian.log',date('Y-m-d H:i:s').'-------后台id'.SELLERUID.'修改团长id'.$tixian['head_id'].'--'.$tixian['member_id'].'--原金额:'.$money.'修改为:'. $_POST['money'].'备注:'.$_POST['remarks'].PHP_EOL, FILE_APPEND);
			$code =1;
		}else{
			$code =0;
		}
		$community_tixian_fee = D('Seller/Front')->get_config_by_name('community_tixian_fee');
		$service_charge = round( ($_POST['money'] * $community_tixian_fee) /100,2);
		$money_charge =$_POST['money']-$service_charge;
		$res =array(
			'code'=>$code,
			'money'=>$_POST['money'],
